...
import { registerAthlete } from "../../api/athlete";
import { showSuccess, showError } from "../../utils/toast";
import type { Athlete, Coach } from "../../types";

const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export function CreateAthleteModal({ coaches, onClose, onDone }: {
    coaches: Coach[];
    onClose: () => void;
    onDone: (athlete: Athlete) => void;
}) {
    ...
    const canSubmit = firstName.trim().length > 0 && emailRegex.test(email) && password.length >= 1 && coachId !== '';

    async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        setBusy(true);
        try {
            const athlete = await registerAthlete(firstName, email, password, coachId);
            showSuccess('Atleta criado com sucesso!');
            onDone(athlete);
            onClose();
        } catch {
            showError('Erro ao criar atleta, tente novamente.');
        } finally {
            setBusy(false);
        }
    }
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center">
            <div className="bg-white rounded-2xl p-7 w-full max-w-[420px] shadow-[0_20px_60px_rgba(0,0,0,.2)]"
                ...
                <div className="flex items-center justify-between mb-1.5">
                    <h2 className="font-head text-[18px] font-bold">Novo Atleta</h2>
                ...
                <p className="text-muted text-[13px] leading-relaxed mb-6">
                    Uma conta de usuário será criada e vinculada ao treinador selecionado.
                </p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="font-head text-sm font-semibold">Treinador responsável</label>
